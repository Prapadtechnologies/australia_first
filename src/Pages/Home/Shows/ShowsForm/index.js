...
const ShowsForm = (props) => {
  ...
  const renderStep1Form = (errors, touched) => {
  ...
  const renderStep2Form = (errors, touched,values) => {
    return <div className="flex flex-col gap-4 mt-4">
        <div className="flex flex-row gap-4 items-center">
            ...
            />  
            <div className="flex justify-center items-center bg-primary w-8 h-8 border border-secondary rounded-md cursor-pointer" onClick={() => setFormStep(1)}>
              <img
                src={AppImages.edit}
    ...
    >
      {({
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <div className="mx-4">
           {
            formStep === 1 && renderStep1Form(errors,touched)
           }
            {
            ...
            <div className="flex flex-row gap-4 mt-6 justify-center items-center">
              <div
                className={`${formStep === 1 ? 'bg-primary' : 'bg-white'} h-7 w-7 flex justify-center items-center border rounded-md cursor-pointer`}
                onClick={() => setFormStep(1)}
              >
                <p className={`font-pop text-xs ${formStep === 1  ?  'text-white' : 'text-primary'}`}> 1</p>
              </div>
              <p className="text-primary font-pop">>></p>
              <div
                className={`${formStep === 2 ? 'bg-primary' : 'bg-white'} h-7 w-7 flex justify-center items-center border rounded-md cursor-pointer`}
                onClick={() => setFormStep(2)}
              >
                <p className={`font-pop text-xs ${formStep === 2  ?  'text-white' : 'text-primary'}`}> 2</p>
              </div>
            </div>
          </div>
        </Form>
